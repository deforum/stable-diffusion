...
def DeforumAnimArgs():
    ...
    overwrite_extracted_frames = True #@param {type:"boolean"} # If set to false, it doesn't extract frames from a video whose frames have already been extracted
...
def vid2frames(video_path, frames_path, n=1, overwrite=True):      
    if not os.path.exists(frames_path) or overwrite: 
      ...
      while success:
        if count % n == 0:
            cv2.imwrite(frames_path + os.path.sep + "%04d.jpg" % count, image)     # save frame as JPEG file
        success,image = vidcap.read()
        count += 1
